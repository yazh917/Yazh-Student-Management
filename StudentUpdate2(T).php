<?php
    ...
    echo "<script>alert('No Student ID specified'); window.location.href='StudentView(T).php'; </script>";
    exit;
}
	...
	if(isset($_POST['StudentUpdate'])) {
		...
		$query= "INSERT INTO student (StudentID, StudentName, Age, Gender, CourseName, PasswordST, EmailST, Phone) VALUES ('$newID', '$name', '$age', '$gender', '$course', '$password', '$email', '$phone' )";

		$result2=mysqli_query($data, $query);

		if($result2) {

			echo "<script>alert ('Upadate Success'); window.location.href='StudentView(T).php'; </script>";
		} else {

			echo "<script>alert ('Update Failed'); window.location.href='StudentUpdate(T).php'; </script> ";
		}
	}
...
<head>
	...
	<link rel="stylesheet" href="StudentUpdate2(T).css">
